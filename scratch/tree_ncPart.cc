...
receiveCbSink(Ptr<const Packet> p)
{
	...
	// at first packet from C2 switch off NC
	Config::Set("/NodeList/*/ApplicationList/*/$CsClusterApp/NcEnable", BooleanValue(false));
}
...
int main(int argc, char *argv[])
{
	...
	clusterHelper.SetClusterAppAttribute("NcIntervalDelay", TimeValue(MilliSeconds(0)));
	...
	sinkApp->TraceConnectWithoutContext("Rx", MakeCallback(&receiveCbSink));
