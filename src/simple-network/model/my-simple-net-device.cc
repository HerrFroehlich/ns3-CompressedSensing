...
MySimpleNetDevice::MySimpleNetDevice() : m_channel(0),
                                         m_node(0),
                                         m_mtu(0xffff),
                                         m_ifIndex(0),
                                         m_linkUp(false)
...
bool MySimpleNetDevice::SimpleTransmit(Ptr<Packet> p)
{
  ...
  if (m_queue->Enqueue(Create<QueueItem>(p)))
  {
    if (TransmitCompleteEvent.IsRunning())
      NS_LOG_WARN("Enqueuing packet! Are you sending packets to fast?");
...
void MySimpleNetDevice::SetChannel(Ptr<MySimpleChannel> channel)
{
...
void MySimpleNetDevice::SetReceiveErrorModel(Ptr<ErrorModel> em)
{
  NS_LOG_FUNCTION(this << em);
  m_receiveErrorModel = em;
}

void MySimpleNetDevice::SetIfIndex(const uint32_t index)
{
...
void MySimpleNetDevice::SetAddress(Address address)
{
...
bool MySimpleNetDevice::SetMtu(const uint16_t mtu)
{
...
bool MySimpleNetDevice::IsLinkUp(void) const
{
  NS_LOG_FUNCTION(this);
  return m_linkUp;
}
void MySimpleNetDevice::AddLinkChangeCallback(Callback<void> callback)
{
  return;
}
bool MySimpleNetDevice::IsBroadcast(void) const
{
  NS_LOG_FUNCTION(this);
  return false;
}
...
bool MySimpleNetDevice::IsMulticast(void) const
{
  NS_LOG_FUNCTION(this);
  return false;
}
...
bool MySimpleNetDevice::IsPointToPoint(void) const
{
  NS_LOG_FUNCTION(this);
  return true;
}

bool MySimpleNetDevice::IsBridge(void) const
{
  NS_LOG_FUNCTION(this);
  return false;
}

bool MySimpleNetDevice::Send(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
...
bool MySimpleNetDevice::SendFrom(Ptr<Packet> p, const Address &source, const Address &dest, uint16_t protocolNumber)
{
  return SimpleTransmit(p);
}

void MySimpleNetDevice::TransmitComplete()
{
  NS_LOG_FUNCTION(this);
...
void MySimpleNetDevice::SetNode(Ptr<Node> node)
{
  NS_LOG_FUNCTION(this << node);
  m_node = node;
}
bool MySimpleNetDevice::NeedsArp(void) const
{
  NS_LOG_FUNCTION(this);
  return false;
}
void MySimpleNetDevice::SetReceiveCallback(NetDevice::ReceiveCallback cb)
{
  NS_LOG_FUNCTION(this << &cb);
  m_rxCallback = cb;
}

void MySimpleNetDevice::DoDispose(void)
{
  NS_LOG_FUNCTION(this);
  m_channel = 0;
  m_node = 0;
...
void MySimpleNetDevice::SetPromiscReceiveCallback(PromiscReceiveCallback cb)
{
  NS_LOG_FUNCTION(this << &cb);
  m_promiscCallback = cb;
}

bool MySimpleNetDevice::SupportsSendFrom(void) const
{
  NS_LOG_FUNCTION(this);
  return false;
}
...
void MySimpleNetDevice::SetQueue(Ptr<Queue> q)
{
