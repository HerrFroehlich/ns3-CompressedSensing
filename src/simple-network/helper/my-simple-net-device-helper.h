/**
* \file my-simple-net-device-helper.h
*
...
* 
*/
#ifndef MY_SIMPLE_NETDEVICE_HELPER_H
#define MY_SIMPLE_NETDEVICE_HELPER_H
...
class MySimpleNetDeviceHelper
{
public:
  /**
   * Construct a MySimpleNetDeviceHelper.
   */
  MySimpleNetDeviceHelper ();
  virtual ~MySimpleNetDeviceHelper () {}
   ...
   * SimpleNetDevice created through MySimpleNetDeviceHelper::Install.
   */
   ...
   * by MySimpleNetDeviceHelper::Install
   */
   ...
   * MySimpleNetDeviceHelper::SetChannelAttribute, an ns3::SimpleNetDevice with the attributes
   * configured by MySimpleNetDeviceHelper::SetDeviceAttribute and then adds the device
   ...
   * MySimpleNetDeviceHelper::SetDeviceAttribute and then adds the device to the node and
   ...
   * \returns A container holding the added net device.
   */
  NetDeviceContainer Install (Ptr<Node> node, Ptr<MySimpleChannel> channel) const;

  /**
   * This method creates an ns3::SimpleChannel with the attributes configured by
   * MySimpleNetDeviceHelper::SetChannelAttribute.  For each Ptr<node> in the provided
   * container: it creates an ns3::SimpleNetDevice (with the attributes
   * configured by MySimpleNetDeviceHelper::SetDeviceAttribute); adds the device to the
   ...
   * MySimpleNetDeviceHelper::SetDeviceAttribute); adds the device to the node; and attaches
   ...
   * \returns A container holding the added net devices.
   */
  NetDeviceContainer Install (const NodeContainer &c, Ptr<MySimpleChannel> channel) const;
...
private:
   ...
   */
  Ptr<NetDevice> InstallPriv (Ptr<Node> node, Ptr<MySimpleChannel> channel) const;
...
#endif  //MY_SIMPLE_NETDEVICE_HELPER_H
